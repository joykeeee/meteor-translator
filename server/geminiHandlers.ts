...
// Shared by both the local Express server (server.ts) and the Vercel
// serverless functions (api/*.ts), so the two never drift apart.
...
export function handleHealth(): RouteResult {
  return {
    status: 200,
    body: {
      status: 'ok',
...
export function extractTone(p: string): number {
...
export function pinyinToZhuyinServer(pinyinWithTone: string): string {
...
export function localParseToTokens(text: string) {
  try {
...
export async function handleAnalyzeSubtitles(input: {
  text?: string;
  episodeTitle?: string;
  videoUrl?: string;
}): Promise<RouteResult> {
  try {
    const { text, episodeTitle, videoUrl } = input;

    if (!text && !videoUrl) {
      return { status: 400, body: { error: 'Text or video info is required' } };
    }
...
[
  {
    ...
    "speaker": "Speaker Name"
        ...
        if (raw) {
          ...
          return { status: 200, body: { lines: enriched, source: `gemini (${modelUsed})` } };
        }
      } catch (geminiErr: any) {
    ...
    return { status: 200, body: { lines: fallbackLines, source: 'local' } };
  } catch (err: any) {
    console.error('Subtitle analysis error:', err);
    return { status: 500, body: { error: err.message || 'Failed to analyze subtitles' } };
  }
}

export async function handleOcrFrame(input: {
  imageBase64?: string;
  timestamp?: number;
  episodeTitle?: string;
}): Promise<RouteResult> {
  try {
    const { imageBase64, timestamp } = input;
...
If no subtitle text is visible:
{
  ...
  "english": ""
    ...
    return { status: 500, body: { error: err.message || 'OCR processing failed' } };
  }
}
...
export async function handleChat(input: {
  ...
  try {
    const { message, history, currentLine, selectedChar, episodeTitle } = input;
...
`;
    ...
    return { status: 500, body: { error: err.message || 'Chat service error' } };
  }
}
